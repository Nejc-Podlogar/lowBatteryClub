...
MessageBufferHandle_t xMessageBufferMag = NULL;
MessageBufferHandle_t xMessageBufferAcc = NULL;
MessageBufferHandle_t xMessageBufferGyro = NULL;
...
void vTaskMagnetometer(void *pvParameters) {
	...
	for (;;) {
		...
		tMagMeritev[0] = ((float) magMeritev[2]) * 1.5 * 0.001;
		tMagMeritev[1] = ((float) magMeritev[3]) * 1.5 * 0.001;
		tMagMeritev[2] = ((float) magMeritev[4]) * 1.5 * 0.001;

		xMessageBufferSend(xMessageBufferMag, (void *)tMagMeritev, sizeof(tMagMeritev), pdMS_TO_TICKS(100));
...
void vTaskAccel(void *pvParameters) {
	...
	for (;;) {
		...
		tAccMeritev[0] = ((float) accMeritev[2]) * 8 / 65536;
		tAccMeritev[1] = ((float) accMeritev[3]) * 8 / 65536;
		tAccMeritev[2] = ((float) accMeritev[4]) * 8 / 65536;

		xMessageBufferSend(xMessageBufferAcc, (void *)tAccMeritev, sizeof(tAccMeritev), pdMS_TO_TICKS(100));
...
void vTaskGyro(void *pvParameters) {
	...
	for (;;) {
		...
		tGyroMeritev[0] = gyroMeritev[2] * 0.0175;
		tGyroMeritev[1] = gyroMeritev[3] * 0.0175;
		tGyroMeritev[2] = gyroMeritev[4] * 0.0175;

		xMessageBufferSend(xMessageBufferGyro, (void *)tGyroMeritev, sizeof(tGyroMeritev), pdMS_TO_TICKS(100));
...
void vTaskPrint(void *pvParameters) {
	...
	float tmpAccArray[3];
	float tmpMagArray[3];
	float tmpGyroArray[3];
	for (;;) {
		xMessageBufferReceive(xMessageBufferGyro, (void *)&tmpGyroArray, sizeof(tmpGyroArray), 0);
		xMessageBufferReceive(xMessageBufferMag, (void *)&tmpAccArray, sizeof(tmpAccArray), 0);
		xMessageBufferReceive(xMessageBufferAcc, (void *)&tmpMagArray, sizeof(tmpMagArray), 0);

		dolzina = sprintf(buffer, "{MAG:{%.3f %.3f %.3f},ACC:{%.3f %.3f %.3f},GYRO:{%.3f %.3f %.3f}}\n\r",
						(float) tmpAccArray[0], (float) tmpAccArray[1], (float) tmpAccArray[2],
						(float) tmpMagArray[0], (float) tmpMagArray[1], (float) tmpMagArray[2],
						(float) tmpGyroArray[0], (float) tmpGyroArray[1], (float) tmpGyroArray[2]);
...
int main(void) {
	...
	xMessageBufferMag = xMessageBufferCreate(16);
	xMessageBufferAcc = xMessageBufferCreate(16);
	xMessageBufferGyro = xMessageBufferCreate(16);
