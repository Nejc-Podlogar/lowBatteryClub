...
void vTaskPrint(void *pvParameters) {
	...
	for (;;) {
		dolzina = sprintf(buffer, "{MAG:{%.3f %.3f %.3f},ACC:{%.3f %.3f %.3f},GYRO:{%.3f %.3f %.3f}}\n\r",
						(float) tMagMeritev[0], (float) tMagMeritev[1], (float) tMagMeritev[2],
						(float) tAccMeritev[0], (float) tAccMeritev[1], (float) tAccMeritev[2],
						(float) tGyroMeritev[0], (float) tGyroMeritev[1],(float) tGyroMeritev[2]);
