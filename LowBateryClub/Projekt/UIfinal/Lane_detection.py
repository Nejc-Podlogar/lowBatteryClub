...
class LaneDetector:
    def __init__(self):
        ...
        #nastavimo cfg datoteko
        self.cfg_file = abs_path + '/cfg/yolov3.cfg'

        # Nastavimo datoteko z modeli
        self.weight_file = abs_path + '/weights/yolov3.weights'

        #naložimo objekte, ki jih podpiramo
        self.namesfile = abs_path + '/data/coco.names'

        #Nastavimo arhitekturo
        self.m = Darknet(self.cfg_file)

        #naložimo modele
        self.m.load_weights(self.weight_file)

        # naložimo imena objektov
        self.class_names = load_class_names(self.namesfile)

        # Nevronska mreža za yolov3
        #m.print_network()

        # Naloži sliko
        #img = cv2.imread(abs_path + '/golf/10.jpg')

        #self.original_image = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

        ##Spremenimo velikost slike
        #self.resized_image = cv2.resize(self.original_image, (self.m.width, self.m.height))

        ## Nastavimo neko verjetnost, da je zazanani okvir, del objekta
        #self.iou_thresh = 0.4

        ##Iz zazanih delov avtomobila, potem dobimo en ekvir
        #self.nms_thresh = 0.6

        ## Zaznamo objekt
        #self.boxes = detect_objects(self.m, self.resized_image, self.iou_thresh, self.nms_thresh)

        ## Izpišemo ime objekta, ter verjetnost, da je to zaznan objekt
        #print_objects(boxes, class_names)

        ##Prikaže sliko ter predmete najdene na sliki
    ...
    def process(self, image):
        try:
            ...
            #Prikaže sliko ter predmete najdene na sliki
            image = plot_boxes(image, boxes, self.class_names, plot_labels = True)
...
def process_video(dirpath, video_file):
    ...
    processed.write_videofile("test8.mp4", audio=False)
...
if __name__ == "__main__":
    process_video(os.path.abspath(os.getcwd()), "8.mp4")